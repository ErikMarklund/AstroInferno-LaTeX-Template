...
% Monster stat block
\begin{AIMonster}[float*=b,width=\textwidth + 8pt]{Monster Foo}
  \begin{multicols}{2}
    \AIMonsterType{Medium aberration (metasyntactic variable), neutral evil}

    % If you want to use commas in the key values, enclose the values in braces.
    \AIMonsterBasics[
        armor-class = {9 (12 with \emph{mage armor})},
        hit-points  = {\AIDice{3d8 + 3}},
        speed       = {30 ft., fly 30 ft.},
      ]

    \AIMonsterAbilityScores[
        str = 12,
        dex = 8,
        con = 13,
        int = 10,
    ...
    % Traits
    \AIMonsterAction{Innate Spellcasting}
    ...
    \begin{AIMonsterSpells}
      \AIInnateSpellLevel{misty step}
      \AIInnateSpellLevel[3]{fog cloud, rope trick}
      \AIInnateSpellLevel[1]{identify}
    \end{AIMonsterSpells}

    \AIMonsterAction{Spellcasting}
    ...
    \begin{AIMonsterSpells}
      \AIMonsterSpellLevel{blade ward, fire bolt, light, shocking grasp}
      \AIMonsterSpellLevel[1][3]{burning hands, mage armor, shield}
    \end{AIMonsterSpells}

    \AIMonsterSection{Actions}
    \AIMonsterAction{Multiattack}
    ...
    %\AIMonsterMelee calls \AIMonsterAttack with the melee option
    \AIMonsterMelee[
      ...
      plus-dmg=\AIDice{2d6},
      plus-dmg-type=fire,
      or-dmg=\AIDice{1d10+1},
      or-dmg-when=if used with two hands,
      %extra=,
    ]

    %\AIMonsterRanged calls \AIMonsterAttack with the ranged option
    \AIMonsterRanged[
    ...
    % Legendary Actions
    \AIMonsterSection{Legendary Actions}
    ...
    \begin{AIMonsterLegendaryActions}
      \AIMonsterLegendaryAction{Move}{The foo moves up to its speed.}
      \AIMonsterLegendaryAction{Dagger Attack}{The foo makes a dagger attack.}
      \AIMonsterLegendaryAction{Create Contract (Costs 3 Actions)}{The foo presents a contract in a language it knows and waves it in the face of a creature within 10 feet. The creature must make a DC 10 Intelligence saving throw. On a failure, the creature is incapacitated until the start of the foo's next turn. A creature who cannot read the language in which the contract is written has advantage on this saving throw.}
    \end{AIMonsterLegendaryActions}
  \end{multicols}
\end{AIMonster}
